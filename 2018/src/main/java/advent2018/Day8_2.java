...
public class Day8_2 {
    ...
    private static int value(Node node) {
        if (node.children.isEmpty()) {
            ...
            // lazy memoization since no cycles
            Map<Integer, Integer> indexValueMap = new HashMap<>(node.metadata.size());
            int sum = 0;
            for (int metadata : node.metadata) {
                int index = metadata - 1;
                if (index < node.children.size()) {
                    if (!indexValueMap.containsKey(index)) {
                        indexValueMap.put(index, value(node.children.get(index)));
                    }
                    sum += indexValueMap.get(index);
                }
            }
            return sum;
        }
