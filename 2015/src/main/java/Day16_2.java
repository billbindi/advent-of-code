...
public class Day16_2 {
    ...
    private static final AuntSue READING_SUE = new AuntSue(
            -1,
            OptionalInt.of(3),
            OptionalInt.of(7),
            OptionalInt.of(2),
    ...
    private static boolean isRealAuntSue(AuntSue option) {
        if (option.children.isPresent() && option.children.getAsInt() != READING_SUE.children.getAsInt()) {
            return false;
        }
        if (option.cats.isPresent() && option.cats.getAsInt() <= READING_SUE.cats.getAsInt()) {
            return false;
        }
        if (option.samoyeds.isPresent() && option.samoyeds.getAsInt() != READING_SUE.samoyeds.getAsInt()) {
            return false;
        }
        if (option.pomeranians.isPresent() && option.pomeranians.getAsInt() >= READING_SUE.pomeranians.getAsInt()) {
            return false;
        }
        if (option.akitas.isPresent() && option.akitas.getAsInt() != READING_SUE.akitas.getAsInt()) {
            return false;
        }
        if (option.vizslas.isPresent() && option.vizslas.getAsInt() != READING_SUE.vizslas.getAsInt()) {
            return false;
        }
        if (option.goldfish.isPresent() && option.goldfish.getAsInt() >= READING_SUE.goldfish.getAsInt()) {
            return false;
        }
        if (option.trees.isPresent() && option.trees.getAsInt() <= READING_SUE.trees.getAsInt()) {
            return false;
        }
        if (option.cars.isPresent() && option.cars.getAsInt() != READING_SUE.cars.getAsInt()) {
            return false;
        }
        if (option.perfumes.isPresent() && option.perfumes.getAsInt() != READING_SUE.perfumes.getAsInt()) {
            return false;
        }
        return true;
    }
