...
public class Day3_2 {
    ...
    public static void main(String[] args) {
        ...
        for (char dir : input.toCharArray()) {
            if (isSanta) {
                houses.add(santa);
                santa = next(santa, dir);
            } else {
                houses.add(roboSanta);
                roboSanta = next(roboSanta, dir);
            }
            isSanta = !isSanta;
        }
    ...
    private static House next(House start, char input) {
        switch (input) {
            case '<': return new House(start.x-1, start.y);
            case '>': return new House(start.x+1, start.y);
            case 'v': return new House(start.x, start.y-1);
            case '^': return new House(start.x, start.y+1);
            default: throw new IllegalArgumentException("Bad input: " + input);
        }
    }
