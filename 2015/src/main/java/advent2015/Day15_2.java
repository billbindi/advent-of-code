...
public class Day15_2 {
        ...
        for (int first = 0; first <= 100; first++) {
            for (int second = 0; first + second <= 100; second++) {
                for (int third = 0; first + second + third <= 100; third++) {
                    ...
                    if (calorieCount(ingrediants, first, second, third, fourth) == 500) {
    ...
    private static int calorieCount(List<Ingrediant> ingrediants, int first, int second, int third, int fourth) {
        return (ingrediants.get(0).getCalories() * first) +
                (ingrediants.get(1).getCalories() * second) +
                (ingrediants.get(2).getCalories() * third) +
                (ingrediants.get(3).getCalories() * fourth);
    }
